...
BACKTRACK_RELAX_SAME_SHAPE = int(os.getenv("TS_BACKTRACK_RELAX_SAME_SHAPE", "1")) != 0
BACKTRACK_RELAX_SAME_SHAPE_THRESHOLD = float(
    os.getenv("TS_BACKTRACK_RELAX_SAME_SHAPE_THRESHOLD", "0.92")
)
...
# By default we cap each tile to at most one same-shape neighbour.  Setting the
# limit to ``-1`` (via TS_SAME_SHAPE_LIMIT) disables the guard if a looser
# layout is desired.
...
class CFG:
    ...
    BACKTRACK_RELAX_SAME_SHAPE = BACKTRACK_RELAX_SAME_SHAPE
    BACKTRACK_RELAX_SAME_SHAPE_THRESHOLD = BACKTRACK_RELAX_SAME_SHAPE_THRESHOLD
